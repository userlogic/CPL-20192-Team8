...
router.get('/cards', async (req, res) => {
  const tourRequests = await TourRequest.query().eager('[requester, location]').modifyEager('requester', builder => {builder.select('first_name')});
  res.json(tourRequests)
})
...
router.post('/', async (req, res) => {
    ...
    const location_id_record = await Location.query().where('name', '=', newTourRequestRaw['city']);
